...
void MECompare( TString currentfile = "new.root",
                ...
                bool Normalize = false, Double_t scale1 = 1., Double_t scale2 = 1.)
{
  ...
  for ( unsigned int index = 0; index < theList.size() ; index++ ) {
    ...
    MEComparePlot(href_, hnew_, currentfile, referencefile, theDir, theList[index], Normalize, scale1 ,scale2 ); 

  }
 
}

void MEComparePlot(TH1 * href_, TH1 * hnew_, TString currentfile, TString referencefile, TString theDir, TString theHisto, bool Normalize, Double_t scale1, Double_t scale2 )
{
 ...
 if ( href_ && hnew_ ) {
   ...
   // Draw reference
   pad1->cd();
   if ( Normalize ) { href_->Sumw2(); href_->DrawNormalized("e1",scale1); }
   else { href_->DrawCopy("e1"); }
   titte->DrawTextNDC(0.,0.02, referencefile);

   // Draw new
   pad2->cd();
   if ( Normalize ) { hnew_->Sumw2(); hnew_->DrawNormalized("e1",scale2); }
   else { hnew_->DrawCopy("e1"); }
   ...
   pad3->SetGridy();
   if ( Normalize ) {
     href_->DrawNormalized("e1",scale1);
     hnew_->DrawNormalized("e1same",scale2);
   }
   else {
     href_->DrawCopy("e1");
